...
from . import xsm_spec_code as xsm_spec

__all__ = ["NustarLoader", "StixLoader", "RhessiLoader", "XsmLoader", "CustomLoader", "rebin_any_array"]
...
class XsmLoader(InstrumentBlueprint):
    """
    Loader specifically for XSM spectral data.
    ...
    def _load1spec(self, f_pha, f_arf, f_rmf, channel_bins=None, photon_bins=None):
            ...
            counts_err = np.sqrt(stat_err**2 + (syst_err*counts)**2)
            ...
            return {"photon_channel_bins": photon_bins,
                    "photon_channel_mids": np.mean(photon_bins, axis=1),
                    "photon_channel_binning": phot_channel_binning,
                    "count_channel_bins": count_bins,
                    "count_channel_mids": np.mean(count_bins, axis=1),
                    "count_channel_binning": count_channel_binning,
                    "counts": self._counts_perspec,
                    "count_error": self._counts_err_perspec,
                    "count_rate": self._count_rate_perspec,
                    "count_rate_error": self._count_rate_error_perspec,
                    "effective_exposure": self._lvt_perspec,
                    "srm": srm,
                    "extras": {"pha.file": f_pha,
                               "arf.effective_area": eff_area,
                               "rmf.file": f_rmf,
