...
class HXI_OSPEXLoader(instruments.InstrumentBlueprint):
    """
    Loader specifically for processed HXI data ready for use with OSPEX.

    HXI_OSPEXLoader Specifics
    ----------------------
    ...
    lightcurve : energy_ranges (list of length=2, lists of length=2 lists, or None), axes (axes object or None)
            Plots the HXI time profile in the energy range given and on the axes provided. Default behaviour (energy_ranges=None)
    ...
    spectrogram : axes (axes object or None) and any kwargs are passed to matplotlib.pyplot.imshow
            Plots the HXI spectrogram of all the data.
    ...
    def __init__(self, spectrum_file, srm_file, custom_channel_bins=None, custom_photon_bins=None, **kwargs):
        ...
        self._construction_string = f"HXI_OSPEXLoader(spectrum_file={spectrum_file},srm_file={srm_file},custom_channel_bins={custom_channel_bins},custom_photon_bins={custom_photon_bins},**{kwargs})"
        ...
        self._systematic_error = 0 #percentage e.g 10% = 0.1

    @property
    def systematic_error(self):
        """Adding systematic error to count_error in quadrature"""
        return self._systematic_error

    @systematic_error.setter
    def systematic_error(self, sys_err):
        self._systematic_error = sys_err
        self._update_event_data_with_times()

    def _getspec(self, spectrum_fn):
        """Load in HXI spectral data.

        Parameters
        ----------
        spectrum_fn : str
                String for the HXI spectral file under investigation.
    ...
    def _get_spec_file_info(self, spec_file):
        """
        Read HXI spectrogram fits file and extracts useful information from it.
        ...
        spec_file : `str`, `file-like` or `pathlib.Path`
                HXI spectral fits file (see `~astropy.fits.io.open` for details)
    ...
    def _spec_file_units_check(self, hxi_dict):
        """Make sure HXI count data is in the correct units.
        ...
        ----------
        hxi_dict : dict
                Dictionary containing all HXI spectral file information.
        ...
        if hxi_dict["1"][0]["TTYPE1"] == "RATE":
            cts_rates = hxi_dict["1"][1]["RATE"]
            cts_rate_err = hxi_dict["1"][1]["STAT_ERR"]
        else:
            raise ValueError("I don't know what units HXI has.")

        return cts_rates, cts_rate_err

    def _getsrm(self, f_srm):
        """Return all HXI SRM data needed for fitting.
        ...
        ----------
        f_srm : str
                String for the HXI SRM spectral file under investigation.
    ...
    def _get_srm_file_info(self, srm_file: str):
        ...
        ----------
        srm_file : str
                String for the HXI SRM spectral file
        ...
        low_photon_bins = srm_file_dat[1]["data"]["ENERG_LO"]
        high_photon_bins = srm_file_dat[1]["data"]["ENERG_HI"]
        photon_bins = np.column_stack((low_photon_bins, high_photon_bins)).to_value(u.keV)

        geo_area = srm_file_dat[0]["header"]["GEOAREA"]


        channels = srm_file_dat[2]["data"]
        channel_bins = np.column_stack((channels["E_MIN"], channels["E_MAX"])).to_value(u.keV)

        srm = srm_file_dat[1]["data"]["MATRIX"].data * np.diff(channel_bins, axis=1).flatten() * geo_area

        return dict(channel_bins=channel_bins, photon_bins=photon_bins, srm=srm)
    ...
    def data2data_minus_background(self, boolean):
        ...
        the event time data minus the background data and fitting with just model; this is the way HXI/OSPEX analysis has been
        ...
        if boolean:
            ...
            new_cts_err = np.sqrt(
                ...
                ** 2
                + (
                    self._full_data["count_error"] * self._systematic_error
                )
                ** 2
            )
    ...
    def _update_event_data_with_times(self):
        ...
        if np.any(np.atleast_1d(self.systematic_error) > 0):
            print()
            self._loaded_spec_data["count_error"] = (
                counts_err := np.sqrt(counts_err**2 + (self._systematic_error * counts) ** 2)
            )
        ...
        effective_exposure = self._loaded_spec_data["effective_exposure"]
        channel_bin = self._loaded_spec_data["count_channel_binning"]

        # calculate new count rates and errors. Since we are ading systematic error, then count rates have to be calculated from counts
        self._loaded_spec_data["count_rate"] = counts / effective_exposure / channel_bin
        self._loaded_spec_data["count_rate_error"] = counts_err / effective_exposure / channel_bin
    ...
    def _update_bg_data_with_times(self, bkg_spec):
    ...
    def lightcurve(self, energy_ranges=None, axes=None, rebin_time=1):
        """Creates a HXI lightcurve.

        Helps the user see the HXI time profile. The defined event time (defined either through `start_event_time`,
        ...
        ar = HXILoader(pha_file=spec_file, srm_file=srm_file)
    ...
    def spectrogram(self, axes=None, rebin_time=1, rebin_energy=1, **kwargs):
        """Creates a HXI spectrogram.

        Helps the user see the HXI time and energy evolution. The defined event time (defined either through
        ...
        --------
        # use the class to load in data
        ar = HXI_OSPEXLoader(pha_file=spec_file, srm_file=srm_file)
        ...
        ax.set_title("HXI Spectrogram [Counts s$^{-1}$]")
...
class BackgroundHandler:
    ...
    def __init__(self, inst_object, energy_range: tuple, time_ranges: list[tuple[float, float]], order: int = 2):
        ...
        times : array of shape (n_times,)
            Time midpoints or start times of background spectra.
        background_counts : array of shape (n_times, n_energy_bins)
    ...
    def format_times(self, time_ranges):
        ...
        for i, time_range in enumerate(time_ranges):
            if isinstance(time_range[0], str) and isinstance(time_range[1], str):
                # string format
                _t_start = Time(time_range[0], format=self.inst_object._time_fmt, scale=self.inst_object._time_scale)
                _t_end = Time(time_range[1], format=self.inst_object._time_fmt, scale=self.inst_object._time_scale)

            else:
                # don't know what to do, print helpful(?) statement and don't do anything
                self.inst_object._req_time_fmt()
                return

            _set_bg_etime = (
                (not hasattr(self, "_start_background_time"))
                or (type(_t_start) is type(None))
                or (type(_t_end) is type(None))
                or (
                    _t_start < _t_end)
                )
            if _set_bg_etime:
                _time_ranges.append((_t_start, _t_end))
            elif self.inst_object.__warn:
                self.inst_object.__time_warning()
        if len(_time_ranges) == len(time_ranges):
            return _time_ranges
        else:
            warnings.warn(
    ...
    def get_spec_data(self, sum_bins=False, avg_bins=False):
        ...
        for time_range in self.time_ranges:
            count_rate = self.inst_object._data_time_select(
                        stime=time_range[0], full_data=self.inst_object._count_rate_perspec, etime=time_range[1]
                    )
