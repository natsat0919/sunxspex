...
class HXI_OSPEXLoader(instruments.InstrumentBlueprint):
    ...
    def __init__(self, spectrum_file, srm_file, custom_channel_bins=None, custom_photon_bins=None, **kwargs):
        ...
        self.background_subtraction = None #[{"energy_band":[], ""}, ]
        # self._start_background_times, self._end_background_times = None, None
    ...
    def _update_bg_data_with_times(self):
        ...
        if (type(self._start_background_time) is not type(None)) and (
            ...
            if np.any(np.atleast_1d(self.systematic_error) > 0):
                self._loaded_spec_data["count_error"] = (
                    bkg_counts_err := np.sqrt(bkg_counts_err**2 + (self._systematic_error * bkg_counts) ** 2)
                )
            ...
            bkg_effective_exposure = self._loaded_spec_data["extras"]["background_effective_exposure"]
            count_channel_binning = self._loaded_spec_data["count_channel_binning"]
            
            # calculate new count rates and errors
            self._loaded_spec_data["extras"]["background_rate"] = (
                bkg_counts
                / bkg_effective_exposure
