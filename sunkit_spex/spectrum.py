...
def rebin_array(array, old_bins, new_bins, combine_by="sum"):
    ...
    for nb in new_bins:
        ...
        if combine_by == "sum":
            new_binned_data.append(np.sum(array[np.where((nb[0] <= old_bins[:, 0]) & (nb[-1] >= old_bins[:, -1]))], axis=0))
        elif combine_by == "mean":
            new_binned_data.append(np.mean(array[np.where((nb[0] <= old_bins[:, 0]) & (nb[-1] >= old_bins[:, -1]))], axis=0))
        elif combine_by == "quadrature":
            new_binned_data.append(np.sqrt(np.sum(array[np.where((nb[0] <= old_bins[:, 0]) & (nb[-1] >= old_bins[:, -1]))]**2, axis=0)))
...
class SpectrometerResponseMatrix(Response):
    def __init__(self, matrix, count_bins=None, photon_bins=None):
        self.matrix = matrix
    ...
    def rebin(self, new_photon_bins=None, new_count_bins=None, axis="photon"):

        if axis != "count":
            srm_rebin = rebin_array(array=self.matrix, old_bins=self.photon_bins, new_bins=new_photon_bins, combine_by="mean")
        if axis != "photon":
            srm_rebin = rebin_array(array=self.matrix.T, old_bins=self.count_bins, new_bins=new_count_bins, combine_by="sum").T  # need to go along columns so .T then .T back
...
#MY SUGGESTION
#NuSTAR ONLY
class ResponseMatrixBase(Response):
    def __init__(self, matrix, unit, count_bins=None, photon_bins=None):
...
class  AuxiliaryResponse(ResponseMatrixBase):

    def rebin(self, new_photon_bins, new_count_bins, axis="photon"):
        """
        Available axes values: photon, count, photon_and_count
        """
        if axis != "count":
            arf_rebin = rebin_array(array=self.matrix.value, old_bins=self.photon_bins, new_bins=new_photon_bins, combine_by="mean")
            arf_rebin =  arf_rebin * self.unit
        else:
            arf_rebin = self.matrix
...
class  ResponseMatrix(ResponseMatrixBase):
    ...
    def rebin(self, new_photon_bins, new_count_bins, axis="photon"):
        ...
        """
        if axis != "count":
            rmf_rebin = rebin_array(array=self.matrix, old_bins=self.photon_bins, new_bins=new_photon_bins, combine_by="mean")
        if axis != "photon":
            rmf_rebin = rebin_array(array=self.matrix.T, old_bins=self.count_bins, new_bins=new_count_bins, combine_by="sum").T  # need to go along columns so .T then .T back
            
        return rmf_rebin * self.unit

class NustarSpectrometerResponseMatrix(Response):

    def __init__(self, arf_matrix, rmf_matrix, photon_bins, count_bins, arf_updated=None, rmf_updated=None, srm_updated=None):
        self.arf = AuxiliaryResponse(arf_matrix, unit=u.m**2, photon_bins= photon_bins, count_bins=count_bins)
        self.rmf = ResponseMatrix(rmf_matrix, unit=u.count/u.photon, photon_bins= photon_bins, count_bins=count_bins)
        self.srm = self.compute_srm(self.arf.matrix, self.rmf.matrix)
        self.arf_updated = arf_updated
        self.rmf_updated = rmf_updated
        self.srm_updated = srm_updated

    def compute_srm(self, arf, rmf):
        srm = arf[:, None]*rmf
        return srm
    
    def rebin(self, new_photon_bins=None, new_count_bins=None, axis="photon"):

        self.arf_updated = self.arf.rebin(new_photon_bins, new_count_bins, axis=axis)
        self.rmf_updated = self.rmf.rebin(new_photon_bins, new_count_bins, axis=axis)

        self.srm_updated = self.compute_srm(self.arf_updated, self.rmf_updated)
